...
`helpers.hselect_input_output.add_file_type_filter_args(parser, file_types_default="py,ipynb,md")`
...
`helpers.hparser.add_bool_arg(parser, "run_diff_script", default_value=True)`
...
`helpers.hparser.add_verbosity_arg(parser)`
...
`helpers.hparser.add_json_output_metadata_args(parser)`
...
`helpers.hselect_input_output.add_input_output_args(parser)`
...
`helpers.hselect_input_output.add_dst_dir_arg(parser, dst_dir_required=False, dst_dir_default=".")`
...
`helpers.hselect_input_output.add_limit_range_arg(parser)`
...
`helpers.hselect_input_output.add_multi_file_args(parser)`
...
`helpers.hselect_action.add_action_arg(parser, valid_actions=["a", "b", "c"], default_actions=["a", "b"])`
...
`helpers.hmarkdown_select.add_select_arg(parser, required=False)`
...
`helpers.hmarkdown_select.add_rule_cli_arg(group)`
...
`helpers.hcache_simple.add_cache_control_arg(parser)`
...
`helpers.hdaemon.add_daemon_arg(parser)`
...
`helpers.hdocker.add_open_arg(parser)`
...
`helpers.hdocker.add_dockerized_script_arg(parser)`
...
`helpers.hjoblib.add_parallel_processing_arg(parser, num_threads_default="1")`
...
`helpers.hllm_cli.add_llm_prompt_arg(parser)`
...
`helpers.hllm_cli.add_llm_args(parser)`
...
`helpers.hs3.add_s3_args(parser)`
...
`config_root.config.config_utils.add_config_override_args(parser)`
...
`dev_scripts_helpers.dockerize.lib_pandoc.add_pandoc_backend_arg(parser, default="auto")`
