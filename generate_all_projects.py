...
"""
Process all Lesson* files in input directory and generate summaries and projects.
...
def find_lesson_files(input_dir: str) -> List[str]:
    ...
    """
    hdbg.dassert(os.path.isdir(input_dir), f"Directory does not exist: {input_dir}")
    ...
    lesson_files.sort()
    _LOG.info(f"Found {len(lesson_files)} Lesson* files: {lesson_files}")
    return lesson_files
...
def generate_summary(
    ...
    cmd_parts = [
        ...
        "--action", "summarize", 
        "--out_file", out_file,
        "--max_num_bullets", str(max_num_bullets),
    ]
    ...
    cmd = " ".join(cmd_parts)
    _LOG.info(f"Running summary command: {cmd}")
...
def generate_projects(in_file: str, output_dir: str) -> str:
    ...
    _LOG.info(f"Running projects command: {cmd}")
...
def process_all_lessons(
    ...
    if not lesson_files:
        _LOG.warning(f"No Lesson* files found in {input_dir}")
        return
    
    # Process each lesson file
    for lesson_file in lesson_files:
        _LOG.info(f"Processing {lesson_file}")
        ...
        try:
            ...
            _LOG.info(f"Generated summary: {summary_file}")
            
            # Generate projects
            projects_file = generate_projects(lesson_file, output_dir)
            _LOG.info(f"Generated projects: {projects_file}")
            
        except Exception as e:
            _LOG.error(f"Failed to process {lesson_file}: {e}")
            raise
