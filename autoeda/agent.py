"""
LangChain single-node agent template with OpenAI API integration and .env support.

This refactors the original LangGraph single-node graph template to use LangChain's Runnable workflow and standard components.

Import as:

from autoeda_agent_langchain import AutoEDAAgent
"""
...
import autoeda.raw_data_analyzer as lsardaan
import autoeda.schema_parser as lsagscpa
...
"""
        ...
        total_rows = state.raw_data_result.get('total_rows', 0) if state.raw_data_result else 0
        total_columns = state.raw_data_result.get('total_columns', 0) if state.raw_data_result else 0
        file_path = state.raw_data_result.get('file_path', 'unknown') if state.raw_data_result else 'unknown'
        
        # Determine dataset characteristics for targeted analysis
        dataset_size = "small" if total_rows < 1000 else "medium" if total_rows < 100000 else "large"
        dataset_width = "narrow" if total_columns < 10 else "wide" if total_columns < 50 else "very wide"
...
• Column breakdown:"""
        ...
        if dataset_width == "wide" or dataset_width == "very wide":
            analysis_focus += ", feature selection and dimensionality reduction"

        prompt = f"""You are an expert AutoEDA assistant. Analyze this {dataset_size}, {dataset_width} dataset:
...
Based on these characteristics, provide targeted recommendations:

1. **Data Quality Assessment**: Identify specific validation rules, missing value patterns, and potential anomalies to investigate

2. **Statistical Analysis Plan**: Recommend appropriate statistical methods and exploratory techniques for this dataset size and structure

3. **Visualization Strategy**: Suggest specific chart types and visualization approaches that work best for this data profile

4. **Risk Areas**: Highlight potential data quality issues, biases, or limitations to investigate based on the schema and size

5. **Next Steps**: Provide a prioritized action plan for the exploratory data analysis process

Keep recommendations practical and specific to the dataset characteristics."""
...
def analyze_raw_data(state: State, config: Configuration) -> State:
    """
...
# #############################################################################
# LangChain Agent as RunnableSequence
# #############################################################################
...
class AutoEDAAgent:
    ...
    def __init__(self, config: Optional[Configuration] = None):
        ...
        self.workflow = (
            RunnableSequence()
            .add(RunnableLambda(lambda state: analyze_raw_data(state, self.config)))
            .add(RunnableLambda(lambda state: parse_schema(state, self.config)))
            .add(RunnableLambda(lambda state: call_model(state, self.config)))
        )
...
# ))
# initial_state = State(file_path="mydata.csv")  # schema_content will be auto-generated from the file
