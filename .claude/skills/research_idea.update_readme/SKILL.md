...
- Find every file in `research/ideas/*.md` modified or added after `<TIMESTAMP>`, or
  not yet present as a row in the table
- Build the list of files to process, `<FILES>`

## Read the Template

- Read `.claude/templates/research_idea.template.md` to know what each idea file
  should contain

## Update the Table

- For each file in `<FILES>`, add or update its row in `research/ideas/README.md`
  using the column order:
  `| File | Links | Status | Assignee | Specs Complete | GitHub Issue | Result |`
- Fill each column to the best of your ability from the file's content and status
  prefix
- Sort all rows by descending `Specs Complete`

## Update the Timestamp

- Update the `// Last update timestamp: "..."` comment at the end of the file to the
  current timestamp
...
- Only touch rows for files in `<FILES>`; do not rewrite rows for unchanged files
  beyond re-sorting the table
