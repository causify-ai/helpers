...
### No Header Structure
- If there is no structure (i.e., the passed text is just a chunk of text)
  then do not use any headers
...
### Use `<MAX_HEADER_LEV>`
- If the user specifies a `<MAX_HEADER_LEV>`, then keep only the structure of the
  paper that has header level lower than `<MAX_HEADER_LEV>`

- For instance if `<MAX_HEADER_LEV>` = 1, then all the text in H1 header needs to
  be summarized, and the output is like:
  ```
  # 1. Hello
