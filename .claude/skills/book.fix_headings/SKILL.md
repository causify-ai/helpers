...
- Given a lecture's `.smd` source and the `.typ` book chapter generated from it (by
  `gen_book_chapter.py --mode typst_aima`), fix the chapter's heading levels to
  follow the "Structural Hierarchy" rule in `.claude/skills/typst.rules.md`, without
...
- **`* Slide Title`**:
  - `seen_subheading` was `False` at this slide → `= Title`, a real heading (add the
    line if it's missing, replace it if it's currently `#strong[Title]`)
  - `seen_subheading` was `True` at this slide → `#strong[Title]` (add the line if
    missing, replace if currently a `=`/`==`/`===` heading)
