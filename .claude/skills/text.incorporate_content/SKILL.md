...
- The user passes, in this order:
  ...
  2. The target `<TARGET>`: a file (e.g., a book, slides, paper, blog) or a reference
     to a part of a file (e.g., a chapter, section, slide number)
...
- Resolve `<TARGET>`
  - If `<TARGET>` is a reference to a part of a file (e.g., "chapter 3", "slide 12"),
    open that file and jump to that part
  - If `<TARGET>` is a whole file and it belongs to a book / course (e.g.,
    `book_springer/map.md`), read the `map.md` to understand the structure of the
    material covered
...
- [ ] Confirm the user was asked for clarification if the location was ambiguous,
      rather than the location being guessed