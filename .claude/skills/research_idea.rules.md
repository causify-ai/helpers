Conventions for the research idea documents under `research/ideas/*.md`: their file
naming, template, and the `README.md` tracking table
...
- Rename the file (change the prefix) when an idea's status changes; do not add a
  separate status field only
...
- Follow `.claude/skills/markdown.rules.md` and `.claude/skills/text.rules.md`
  when writing the text as bullet points
- Fit text wrapping it in 85 character lines
...
- The end of `research/ideas/README.md` carries a comment recording when the table
  was last regenerated, e.g. `// Last update timestamp: "Aug 3 10:54"`
- Update this comment whenever the table changes
- Use the comment to find which idea files changed since the last update: any file in
  `research/ideas/*.md` modified or added after that timestamp needs a table row

# Examples

- `research/ideas/in_progress.RL_for_pickleball.md`: a research idea fully formatted
  per the template
