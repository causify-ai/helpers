...
- The outline describes what each cell will contain without writing any code: it
  serves as a blueprint for implementation

# Core Goals

- An effective interactive notebook outline should enable:
  1. **Strong intuition**: Help students build mental models through discovery
  2. **Visual explanation**: Use plots, diagrams, and animations to make concepts
     concrete
  3. **Incremental building**: Start simple, add complexity layer by layer
  4. **Interactive exploration**: Let students manipulate parameters and see
     immediate results
...
Each cell in your outline corresponds to a **pair of cells** in the final
notebook:
1. **Markdown cell**: Section header and pedagogical context
2. **Code cell**: Visualization, widgets, and explanatory text
...
- **Interactive widget**:
  ...
  - Description: "Drag to change the unknown true proportion. Notice how the bin
    looks different but you can only ever see samples from it in practice"
- **Key insights**:
  ...
  - Intuition: different populations look different when fully observed (which we
    can't do)
- **Comment box**: "In practice, we never see the full population—only random
  samples from it. This visualization shows what we're trying to infer from those
  samples."
