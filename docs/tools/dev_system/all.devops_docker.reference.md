...
- There are several types of `devops` dirs
  - A `devops` where a container is built, run, and released
    - E.g., `//amp`, `//helpers`, `//optimizer`, `//tutorials`, `//demo`
...
// TODO(ai_gp): Make reference to //helpers here
  ```
  > cd //amp
  > tree.sh -p devops
  devops/
...
- Keep the directories and files in alphabetical order
...
- `dockerignore.dev`: exclude files from the build context when building the
  dev image
  - The dev build context is only `devops/`, so everything else is ignored
- `dockerignore.prod`: exclude files from the build context when building the
  prod image
  - E.g., `docs/`, `.git/`, `ck.infra`
...
- `install_python_packages.sh`: install the Python packages with `poetry` and
  `pip`
- `pip_list.txt`: pinned output of `pip list` capturing the exact Python
  package versions installed in the image, for reproducibility and debugging
...
- `pyproject.python_data_stack.toml`: alternative `pyproject.toml` listing
  only the core Python data-stack packages (e.g., `pandas`, `numpy`,
  `matplotlib`, `scikit-learn`) for images that don't need the full
  dependency set
- `pyproject.toml`: list of package specification for `poetry`
- `update_os.sh`: update the OS package listing and install security updates
  (`apt-get update && apt-get upgrade`)
- `utils.sh`: shared bash functions used by the other `docker_build` scripts
  - E.g., `print_vars()`, `report_disk_usage()`

### `devops/docker_run/`
- `bashrc`: basic configuration for bash shells inside a container
- `docker_setenv.sh`: configure the virtual environment inside the container
...
- `entrypoint.sh`: configures the container on startup
  - E.g., run `docker_setenv.sh`, start dind, setup Git to run inside the
    container
...
- Edit `/devops/docker_build/os_packages/os_packages.txt` to control which
...
- **Important**: While it is technically possible to build the Docker image
  ...
  - `hs3.py` functions like `copy_file_to_s3()` and
    ...
    `install_os_aws_cli.sh`)
  - `lib_tasks_gh.py` functions like `gh_pr_create()`, `gh_workflow_list()`, and
    `gh_issue_get_title()` use GitHub CLI commands (`gh pr create`,
