...
- [Cache Simple](#cache-simple)
  ...
  * [Usage Examples](#usage-examples)
    + [Basic Local Caching](#basic-local-caching)
    + [Binary Data Caching](#binary-data-caching)
    + [Team Cache Sharing via S3](#team-cache-sharing-via-s3)
    + [Per-Function Custom Cache Location](#per-function-custom-cache-location)
    + [Per-Function S3 Configuration](#per-function-s3-configuration)
    + [Manual S3 Sync Workflow](#manual-s3-sync-workflow)
    + [Bidirectional S3 Sync](#bidirectional-s3-sync)
    + [Excluding Session-Specific Parameters](#excluding-session-specific-parameters)
    + [Testing with Mock Cache](#testing-with-mock-cache)
    + [Cache Management and Inspection](#cache-management-and-inspection)
    + [Performance Monitoring](#performance-monitoring)
    + [Global vs Per-Function Configuration](#global-vs-per-function-configuration)
