...
class TestPlaybackInputOutput1(hunitest.TestCase):
    ...
    def helper(self, mode: str, *args: Any, **kwargs: Any) -> None:
        ...
        def get_result_assert_equal(a: Any, b: Any) -> Any:
            ...
            if isinstance(a, dict) and isinstance(b, dict):
                ...
                c.update(b)
                return p.run(c)
            if isinstance(a, config.Config) and isinstance(b, config.Config):
                c = config.Config(update_mode="overwrite")
                c.update(a)
                c.update(b)
                return p.run(c)
            return p.run(a + b)
    ...
    def test15(self) -> None:
        """
        Test for config.Config inputs with check_string.
        """
        # Create inputs.
        a = config.Config([("meta", "meta value 1"), ("list", [1, 2])])
        b = config.Config([("meta", "meta value 2")])
        # Generate, freeze and execute a unit test.
        self.helper("check_string", a=a, b=b)

    def test16(self) -> None:
        """
        Test for config.Config inputs with assert_equal.
        """
        # Create inputs.
        a = config.Config([("meta", "meta value 1"), ("list", [1, 2])])
        b = config.Config([("meta", "meta value 2")])
...
class TestToPythonCode1(hunitest.TestCase):
    ...
    def test_config1(self) -> None:
        ...
        config["var1"] = "val1"
        config["var2"] = config.Config([("var3", 10), ("var4", "val4")])
        self._check(
            config,
            "config.Config.from_python(\"Config([('var1', 'val1'), "
