...
def run_git_recursively(ctx: Any, cmd_: str, dry_run: bool = False) -> None:
    """
...
@task
def git_symlink_perms_set(ctx, dir_name="."):  # type: ignore
    """
...
@task
def git_symlink_perms_reset(ctx, dir_name="."):  # type: ignore
    """
...
@task
def git_pull(ctx, dry_run=False):  # type: ignore
    """
    ...
    :param dry_run: if True, log the commands without executing them
    """
    ...
    try:
        ...
        run_git_recursively(ctx, cmd, dry_run=dry_run)
    finally:
...
# TODO(gp): Seems thin.
@task
def git_fetch_master(ctx, submodules=True, dry_run=False):  # type: ignore
    """
...
@task
def git_add_all_untracked(ctx, exclude_tmp=False, dry_run=False):  # type: ignore
    """
    Add all untracked files to Git.

    :param exclude_tmp: skip `tmp.*` files (e.g.,
        `tmp.precommit_output.txt`) generated by other tasks, such as
        `git_merge_master`
    :param dry_run: if True, log the command without executing it
    """
    hltltaut.report_task(txt=hprint.to_str("dry_run"))
...
@task
def git_submodules_roll_forward(ctx):  # type: ignore
    """
...
def git_branch_create(  # type: ignore
    ...
    :param dry_run: if True, log the actions the script would take without
...
def _delete_branches(
    tag: str, confirm_delete: bool, dry_run: bool = False
) -> None:
    """
    ...
    :param dry_run: if True, log the branches without deleting them
    """
    ...
    if dry_run:
        _LOG.warning(
            "Skipping deletion of %d %s branches (dry run)", len(branches), tag
        )
        return
...
@task
def git_branch_delete_merged(ctx, confirm_delete=True, dry_run=False):  # type: ignore
    """
    Remove (both local and remote) branches that have been merged into master.

    :param dry_run: if True, log which branches would be deleted without
        deleting them
    """
    hltltaut.report_task(txt=hprint.to_str("dry_run"))
    ...
    _delete_branches("local", confirm_delete, dry_run=dry_run)
    _delete_branches("remote", confirm_delete, dry_run=dry_run)
    #
    cmd = "git fetch --all --prune"
    hltltaut.run(ctx, cmd, dry_run=dry_run)
...
@task
def git_branch_rename(ctx, new_branch_name, dry_run=False):  # type: ignore
    """
    ...
    :param dry_run: if True, log the commands without executing them and
        skip the confirmation prompt and PR recreation
    """
    hltltaut.report_task(txt=hprint.to_str("dry_run"))
    ...
    if not dry_run:
        msg = (
            f"Do you want to rename the current branch '{old_branch_name}' "
            f"to '{new_branch_name}'"
        )
    ...
    if pr_info is not None:
        if dry_run:
            _LOG.warning(
                "Skipping PR recreation for '%s' (dry run)", new_branch_name
            )
        else:
            ...
            hltltaut.run(ctx, cmd)
    print("Done" if not dry_run else "Done (dry run)")


@task
def git_branch_get_next_name(  # type: ignore
...
def git_branch_subset_copy(  # type: ignore
    ...
    :param dry_run: if True, log the actions the script would take without
        creating the branch or copying files
    """
    hltltaut.report_task()
...
@task
def git_repo_copy(  # type: ignore
    ctx, file_name, src_git_dir, dst_git_dir, dry_run=False
):
    """
    ...
    :param dry_run: if True, log the copy without performing it
    """
    _ = ctx
    ...
    if dry_run:
        _LOG.warning(
            "Skipping copy of '%s' to '%s' (dry run)", file_name, dst_file_path
        )
        return
...
@task
def git_perms_fix(ctx, check=True, fix=False, dir_name="."):  # type: ignore
    """
...
@task
def git_file_get_version(  # type: ignore
    ctx,
    file_path="",
    count=5,
    dir_name="",
    ...
    Versions are ordered from newest to oldest. Creates a `tmp.git_file_get_version.sh`
    ...
    script_content += "# Generated by: invoke git_file_get_version\n"
    script_content += "\n"
    script_content += f"vim {' '.join(version_files)}\n"
    script_file = "tmp.git_file_get_version.sh"
