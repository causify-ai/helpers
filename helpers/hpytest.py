...
class JUnitReporter:
        ...
        try:
            ...
            for suite in self.xml_data:
                if isinstance(suite, TestSuite):
                    ...
                    self.overall_stats['passed'] += (suite.tests or 0) - (suite.failures or 0) - (suite.errors or 0) - (suite.skipped or 0)
    ...
    def print_detailed_results(self):
        ...
        print(f"collected {self.overall_stats['total_tests']} items{Colors.RESET}")
        print()
        for i, suite in enumerate(self.xml_data):
            ...
            print(f"{Colors.BLUE}Timestamp: {getattr(suite, 'timestamp', 'Unknown')}{Colors.RESET}")
            ...
            for case in suite:
                if isinstance(case, TestCase):
                    ...
                    print(f"  {case.classname}::{case.name} {status_display} ({test_time:.3f}s)")
            # Print suite summary.
            suite_passed = (suite.tests or 0) - (suite.failures or 0) - (suite.errors or 0) - (suite.skipped or 0)
            summary_parts = []
            if suite_passed > 0:
                summary_parts.append(f"{Colors.GREEN}{suite_passed} passed{Colors.RESET}")
            if suite.failures and suite.failures > 0:
                summary_parts.append(f"{Colors.RED}{suite.failures} failed{Colors.RESET}")
            if suite.errors and suite.errors > 0:
                summary_parts.append(f"{Colors.MAGENTA}{suite.errors} error{Colors.RESET}")
            if suite.skipped and suite.skipped > 0:
                summary_parts.append(f"{Colors.YELLOW}{suite.skipped} skipped{Colors.RESET}")
            suite_summary = ", ".join(summary_parts) if summary_parts else "no tests"
            suite_time = getattr(suite, 'time', 0) or 0
            print(f"{Colors.BOLD}Summary: {suite_summary} in {suite_time:.3f}s{Colors.RESET}")
    
    def print_final_summary(self):
        summary_parts = []
        if self.overall_stats['passed'] > 0:
            summary_parts.append(f"{Colors.GREEN}{self.overall_stats['passed']} passed{Colors.RESET}")
        if self.overall_stats['failed'] > 0:
            summary_parts.append(f"{Colors.RED}{self.overall_stats['failed']} failed{Colors.RESET}")
        if self.overall_stats['error'] > 0:
            summary_parts.append(f"{Colors.MAGENTA}{self.overall_stats['error']} error{Colors.RESET}")
        if self.overall_stats['skipped'] > 0:
            summary_parts.append(f"{Colors.YELLOW}{self.overall_stats['skipped']} skipped{Colors.RESET}")
        ...
        elif self.overall_stats['skipped'] > 0 and self.overall_stats['passed'] == 0:
            status_indicator = f"{Colors.YELLOW}{Colors.BOLD}SKIPPED{Colors.RESET}"
        else:
