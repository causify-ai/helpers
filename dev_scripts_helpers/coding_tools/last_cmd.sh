#!/bin/bash
#
# Wrapper for `last_cmd.py` that flushes the current shell's history to disk
# and dumps the last few commands to a tmp file for `last_cmd.py` to read.
#
...
# interactive shell's. This script must be *sourced*, not executed, so that
# `history -a` / `history 10` run in the actual interactive shell instead
# of in a throwaway subshell.
#
# > source last_cmd.sh
# > . last_cmd.sh -n 3
