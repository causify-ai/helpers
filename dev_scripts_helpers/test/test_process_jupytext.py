...
class Test_process_jupytext(hunitest.TestCase):
    ...
    def test_end_to_end(self) -> None:
        ...
        hio.to_file(file_path, py_text)
        # Run processor.
        cmd = f"dev_scripts_devto/notebooks/process_jupytext.py -f {file_path} --action sync 2>&1"
        hsystem.system(cmd)
        cmd = f"dev_scripts_devto/notebooks/process_jupytext.py -f {file_path} --action test 2>&1"
        hsystem.system(cmd)
