...
def _get_files() -> Tuple[str, str]:
    ...
    notebook_file = os.path.join(
        amp_path, "dev_scripts_helpers/notebooks/test/simple_notebook.ipynb"
    )
...
class TestRunNotebook1(hunitest.TestCase):
    ...
    def test_serial1(self) -> None:
        ...
        """
        cmd_opts = [
            "--config_builder 'dev_scripts_helpers.test.test_run_notebook.build_config_list1()'",
            "--num_threads 'serial'",
        ]
        #
        exp_pass = True
...
class TestRunNotebook2(hunitest.TestCase):
    ...
    def test_serial1(self) -> None:
        ...
        """
        cmd_opts = [
            "--config_builder 'dev_scripts_helpers.test.test_run_notebook.build_config_list2()'",
            "--num_threads 3",
        ]
        #
        exp_pass = False
...
class TestRunNotebook3(hunitest.TestCase):
    ...
    def helper(
        ...
        config_builder = (
            f"dev_scripts_helpers.test.test_run_notebook._config_builder({fail})"
        )
        cmd_run_txt = [
            f"{script_path}",
