...
def _parse_markdown_scenes(file_path: str) -> List[Dict[str, str]]:
    """
    Parse text file and extract scenes with visuals, narration, and file
    references.
...
def _generate_video_for_scene(
    ...
    *,
    ...
    :param scene: scene dictionary with visuals, narration,
        negative_prompt, duration_in_secs, file
    :param scene_index: index of the scene for output naming
    :param resolution: video resolution
    :param aspect_ratio: video aspect ratio
    :param default_duration_seconds: default duration if not specified
        in scene
    :param image_file: global reference image file (overrides scene-
        specific file)
    ...
    _LOG.info(
        "Generating video for scene '%s' (duration: %ds): %s",
        title,
        duration_in_seconds,
    ...
    if negative_prompt:
        ...
        _LOG.debug(
            "Using negative prompt for scene '%s': %s", title, negative_prompt
        )
    ...
    _LOG.info(
        "generate_videos_kwargs=\n%s", pprint.pformat(generate_videos_kwargs)
    )
    if dry_run:
    ...
    if effective_image_file:
        ...
        image_obj = {"imageBytes": image_base64, "mimeType": mime_type}
        generate_videos_kwargs["image"] = image_obj
        _LOG.info(
            "Added image to video generation: %s (mime_type: %s)",
            effective_image_file,
            mime_type,
        )
    #
    ...
    while not operation.done:
        _LOG.debug(
            "Waiting for video generation to complete for scene '%s'", title
        )
...
def _generate_videos_from_scenes(
    ...
    *,
    ...
    :param low_res: whether to use low resolution settings for faster
        rendering
    :param dry_run: if True, only print what would be executed
    ...
    for i, scene in enumerate(tqdm.tqdm(scenes, desc="Generating videos")):
        if low_res:
            ...
            output_file = _generate_video_for_scene(
                client, scene, i + 1, image_file=image_file, dry_run=dry_run
            )
        #
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    scenes = hselsio.apply_limit_range(scenes, limit_range, item_name="scenes")
    # Generate videos for all scenes.
    ...
    #
    _LOG.info(
        "Generated %d videos out of %d scenes", len(generated_files), len(scenes)
    )
