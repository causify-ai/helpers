...
Actions can be selectively added or skipped using --action or --skip_action
flags, or restricted to a subset via --clear_actions --action.
...
def _get_directories(
    in_dir: str, *, limit_range: Optional[Tuple[int, int]] = None
) -> List[str]:
    """
...
def _create_directory_table(directories: List[str], out_dir: str) -> None:
    ...
    Maps directory names to owners, departments, and content
    descriptions.
    ...
    directory_info = {
        "All Causify": {"owner": "", "department": "General", "content": ""},
        "Archives (Vielka)": {
            "owner": "Vielka",
            "department": "General",
            "content": "",
        ...
        "Eng": {"owner": "GP", "department": "Engineering", "content": ""},
        "Eng - External": {
            "owner": "GP",
            "department": "Engineering",
            "content": "",
        ...
        "Finance": {"owner": "Brad", "department": "Finance", "content": ""},
        "General": {"owner": "Joel", "department": "General", "content": ""},
        "Grants": {"owner": "", "department": "Finance", "content": ""},
        "HR": {"owner": "", "department": "HR", "content": ""},
        "Investment - TO_REORG": {
            "owner": "Joel",
            "department": "Finance",
            "content": "",
        ...
        "Marketing": {"owner": "Joel", "department": "Marketing", "content": ""},
        "New Market Planning": {
            "owner": "Joel",
            "department": "Strategy",
            "content": "",
        ...
        "Process": {"owner": "GP", "department": "Operations", "content": ""},
        "Product: FinTech apps": {
            "owner": "Eng",
            "department": "Product",
            "content": "",
        },
        "Sales General": {"owner": "Joel", "department": "Sales", "content": ""},
    }
    ...
    for i, directory in enumerate(
        tqdm(directories, desc="Processing directories for table"), 1
    ):
...
def _combine_llm_outputs(directories: List[str], out_dir: str) -> None:
    ...
    Reads all llm_XXX.txt files, matches them with corresponding
    directories, and creates a combined markdown file with directory
    names as H1 headers and content with incremented header levels.
...
def _run_llm_on_file(
    ...
    llm_cmd = (
        f"printf '%s' '{escaped_prompt}' | llm -m gpt-4o-mini -f \"{log_file}\""
    )
    ...
    for i, directory in enumerate(directories, 1):
        ...
        if _ACTION_LLM in actions:
            if os.path.isfile(tree_file):
                ...
                _LOG.warning(
                    "Tree file %s does not exist, skipping LLM processing",
                    tree_file,
                )
        else:
            _LOG.info(
