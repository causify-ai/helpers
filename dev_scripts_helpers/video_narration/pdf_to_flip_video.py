...
def ensure_even_dimensions(w: int, h: int) -> Tuple[int, int]:
    """
    Some encoders prefer even dimensions.

    If odd, add one pixel.
    """
    if w % 2 != 0:
        w += 1
    if h % 2 != 0:
...
def _crossfade_sequence(
    prev_img: Image.Image, next_img: Image.Image, dur: float, fps: int
) -> ImageSequenceClip:
    """
    Create an ImageSequenceClip that linearly crossfades from prev_img to
    next_img over dur seconds.
    """
...
def _make_crossfade_video(
    ...
    for i in range(1, len(norm)):
        trans = (
            _crossfade_sequence(norm[i - 1], norm[i], crossfade, fps)
            if crossfade > 0
            else None
        )
    ...
    final.write_videofile(
        out_path, fps=fps, audio=False, codec="libx264", preset="medium"
    )


def _slide_transition_clip(
    prev_img: Image.Image, next_img: Image.Image, dur: float
):
    """
    Return a CompositeVideoClip that slides next_img from right -> left over
    prev_img (MoviePy 2.x API).
    """
    base_w, base_h = prev_img.size
    base_w, base_h = ensure_even_dimensions(base_w, base_h)
    if next_img.size != (base_w, base_h):
    ...
    comp = CompositeVideoClip(
        [sliding_prev, moving_next], size=(base_w, base_h)
    ).with_duration(dur)
    return comp
...
def _parse() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out", default="out.mp4", help="Output video path (e.g., out.mp4)"
    )
    parser.add_argument(
        "--dpi",
        type=int,
        default=200,
        help="DPI for rasterizing PDF pages (higher = sharper, slower)",
    )
    parser.add_argument(
        "--fps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Seconds each page is fully visible",
    )
    parser.add_argument(
        "--transition",
        type=float,
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    if args.style == "crossfade":
        _make_crossfade_video(
            images,
            page_duration=args.page_duration,
            crossfade=args.transition,
            fps=args.fps,
            out_path=args.out,
        )
    else:
