...
def _format_grid_code(
	widths: List[str], column_contents: List[str]
) -> str:
	"""
...
def _transform_elem(
	...
	if elem.get("t") == "Div" and elem.get("c"):
		children = elem["c"][1]
		transformed_children = [_transform_elem(child, api_version) for child in children]
...
def _transform_ast(ast: Dict[str, Any]) -> Dict[str, Any]:
	...
	transformed_blocks = [_transform_elem(block, api_version) for block in blocks]
