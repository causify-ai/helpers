...
def _summarize_file(
    ...
    *,
    ...
    and writes the result to the output file. Optionally lints the output.
    ...
    # Read the markdown file to be summarized.
    input_content = hio.from_file(input_file)
    _LOG.debug("Read %d characters from input file", len(input_content))
    # Generate system prompt with structure preservation rules and bullet point conventions.
    system_prompt = _get_system_prompt()
    # Send content to LLM for summarization using the specified model.
    ...
    # Optionally apply linting to format the summary according to style guide.
    if lint:
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    input_files = hselsio.parse_input_output_files(args)
