...
# 2. Fill in Article_url / Article_tag / Article_cluster (LLM tagging, costs money)
if [[ 0 == 1 ]]; then
...
# 3. Show situation of CSV vs iCloud Drive (and notes1/bookmarks) — safe, no writes
if [[ 1 == 1 ]]; then
...
# 4. Populate both destinations (same command, drop --dry_run)
if [[ 1 == 1 ]]; then
process_bookmarks.py \
  -i /Users/saggese/src/notes1/bookmarks/update_gsheet_links_from_raindrop.combined_data.csv \
