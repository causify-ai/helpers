...
(including `--dest_type none`), as a version-controlled copy separate from the
user-selectable viewing destination.
...
# Git-tracked dir every processed row's merged summary is also backed up into,
# in addition to whatever `--dest_type` is selected. Always-on and independent
# of `--dest_type` (including `none`): unlike Google Drive/Obsidian, this is a
# version-controlled backup, not a user-selectable viewing destination, so it's
# not exposed as a CLI flag. Tests must override it via `umock.patch.object()`,
# never let it point at this real path.
...
def _main(parser: argparse.ArgumentParser) -> None:
    ...
    if not args.dry_run:
        ...
        hio.create_dir(_GIT_BACKUP_DIR, incremental=True)
    ...
    # Always also back up into the git-tracked bookmarks dir, independent of
    # --dest_type (including --dest_type none): a version-controlled copy of
    #  every processed summary, separate from the user-selectable viewing
    #  destination.
    backup_copied_filenames = _reconcile_destination(
        rows,
        output_dir=args.output_dir,
        dest_dir=_GIT_BACKUP_DIR,
        dry_run=args.dry_run,
    )
    if args.dry_run:
        _LOG.info(
            "[DRY_RUN] Would back up %d row(s) into git dir '%s'",
            len(backup_copied_filenames),
            _GIT_BACKUP_DIR,
        )
    else:
        _LOG.info(
            "Backed up %d row(s) into git dir '%s'",
            len(backup_copied_filenames),
            _GIT_BACKUP_DIR,
        )
