...
- **What It Does**
  ...
  - Useful for understanding the state of your working directory and all
    submodules
...
- **What It Does**
  - `git_branch.sh`: Show detailed information about branches including author
    and date
  - `git_branch_name.sh`: Print just the name of the current branch (useful in
    scripts)
  - `git_branch_point.sh`: Show information about the branch point (common
    ancestor with main)
...
- **What It Does**
  ...
  - `gcl`: Clean the entire client by stashing changes and removing untracked
    files with backup
...
- **What It Does**
  ...
  - `gup.py`: Update git client by stashing, rebasing, and reapplying stashed
    changes
...
- **`gitleaks-rules.toml`**: Custom rules for detecting sensitive information
  patterns (API keys, credentials, tokens, etc.)
