# Fix Plan for TODOs in `instr.md`

- 24 issues below, one per `TODO(ai_gp)` line in `instr.md`
...
### [ ] Move `model` parameter after `*` in `summarize_text_with_llm()`
...
- [ ] PR1: Force `model` to be keyword-only
  - Move `model: str = _SUMMARY_MODEL` to after the `*` in the signature,
    next to `dry_run`

---

### [ ] Convert `version_line` to a raw triple-quote string in `preprocess_notes.py`
...
- [ ] PR1: Rewrite `version_line` as a dedented raw string
...
### [ ] Move `hprint.dedent()` calls into `helper()` in `Test_CustomHelpFormatter_split_lines`
...
### [ ] Move `self.check_string()` call into `helper()` in `Test_notes_to_pdf_latex_cancel`
...
- [ ] PR1: Fold the shared `check_string()` call into `helper()`
...
### [ ] Move `assert_equal` into `helper_mock_hn_api()` in `Test__update_article_urls`
...
- [ ] PR1: Add `expected` param to `helper_mock_hn_api()` and assert inside
  - Change signature to
...
### [ ] Move `assert_equal` into `helper()` in `Test__update_article_clusters`
...
- [ ] PR1: Add `expected` param to `helper()` and assert inside
...
### [ ] Move `assert_equal` into `helper()` in `Test__combine_raindrop_with_gsheet_links`
...
### [ ] Move `assert_equal` into `helper()` in `Test__download_raindrop_data`
...
### [ ] Factor out common mocking code in `Test__is_engine_available`
...
- [ ] PR1: Add a `helper()` taking the two booleans and the expected result
...
### [ ] Factor common code and move `assert_equal` into a helper in `Test_CustomHelpFormatter_format_help`
...
- [ ] PR1: Add a `helper()` for the shared build-and-compare pattern
...
### [ ] Add a helper method in `Test_perform_actions_typ_type`
...
- [ ] PR1: Add a `helper()` for the shared call-and-compare pattern
...
### [ ] Use `hunteuti.capture_sys_calls()` in `Test_process_file_one_shot_with_cc`
...
- [ ] PR1: Swap the direct `hsystem.system` mock for `capture_sys_calls()`
...
### [ ] Use `hunteuti.capture_sys_calls()` in `_DockerFlowTestHelper.set_up_test()`
...
- [ ] PR1: Replace the `hsystem.system` patcher with `capture_sys_calls()`
...
### [ ] Factor common code and use `capture_sys_calls()` in `Test__cleanup_dangling_volumes`
...
- [ ] PR1: Add a `helper()` using `capture_sys_calls()`
...
### [ ] Factor common code and use `capture_sys_calls()` in `Test__cleanup_dangling_images`
...
### [ ] Factor common code and use `capture_sys_calls()` in `Test__cleanup_build_cache`
...
### [ ] Factor common code and use `capture_sys_calls()` in `Test__cleanup_unused_networks`
...
### [ ] Factor out repeated input-file setup in `Test_run_typst_py`
...
- [ ] PR1: Add a small helper for the repeated input-file setup
...
### [ ] Move regex group comments inline via `re.VERBOSE` in `docker_cleanup.py`
...
- [ ] PR1: Convert `_SYSTEM_DF_ROW_RE` to a `re.VERBOSE` pattern
...
### [ ] Convert Typst character-escaping to a loop with shorter comments
...
- [ ] PR1: Replace the 7 `.replace()` calls with a loop
...
### [ ] Clarify and apply "Improve test to follow testing.rules.md" in `test_compress_pdf.py`
...
* Solution
- Ask for clarification before starting: confirm whether the fix should be
  limited to the concrete `assertEqual` -> `assert_equal` swap for string
  comparisons found above, or should be a full `testing.rules.md`
  compliance pass over the whole file (naming, helper usage, section
  comments, etc.)

- [ ] PR1 (minimal scope): Fix string comparisons to use `assert_equal`
  - Replace `self.assertEqual(actual_content, expected_content)` (and any
    other string-vs-string `assertEqual`) with `self.assert_equal(...)` in
    all 4 classes in the file
...
* Solution
- Ask for clarification before starting: confirm whether it is worth
  extending `capture_sys_calls()` for `Popen` now, or whether these 3
  TODOs should simply be left as documented, deferred debt (the TODO text
  already explicitly defers itself on this precondition)
...
* Solution
- Ask for clarification before starting (same question as the sibling
  `Popen` issues: worth extending shared infra now, or leave deferred)
